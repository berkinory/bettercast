...
final class ExtensionHostRuntime {
    ...
    func handle(_ message: ExtensionJSON) throws {
        ...
        default:
            sendError(
                requestID: message["requestID"] as? String, code: "invalidMessage",
                message: "Unsupported host message: \(type)")
        }
    }
    ...
    private func launch(_ message: ExtensionJSON) throws {
        ...
        for (key, value) in [
            "__opencastCommandName": message["commandName"],
            "__opencastExtensionName": message["extensionName"],
            "__opencastCommandMode": message["mode"],
            "__opencastLaunchType": message["launchType"],
            "__opencastSupportPath": message["supportPath"],
            "__opencastAssetsPath": message["assetsPath"],
        ] {
            if let value { jsContext.setObject(value, forKeyedSubscript: key as NSString) }
        }
        jsContext.evaluateScript(Self.bootstrap)
        guard jsContext.exception == nil else {
            throw ExtensionHostError.runtime(
                jsContext.exception?.toString() ?? "Failed to initialize JavaScript runtime.")
        }
    ...
    private func dispatch(_ message: ExtensionJSON) throws {
        guard let jsContext = context, !disposed else {
            sendError(
                requestID: message["requestID"] as? String, code: "runtime", message: "No extension command is running."
            )
            return
        }
        ...
        if let exception = jsContext.exception {
            sendError(
                requestID: message["requestID"] as? String, code: "runtime",
                message: exception.toString() ?? "JavaScript exception")
    ...
    private func resolveCapability(_ message: ExtensionJSON) throws {
        ...
        guard let function = jsContext.objectForKeyedSubscript("__opencastResolveCapability"), function.isObject else {
            return
        }
    ...
    private func handleBridgeMessage(_ rawMessage: String) {
        ...
        if type == "render", let encoded = try? JSONSerialization.data(withJSONObject: message),
            encoded.count > ExtensionHostLimits.maxSnapshotBytes
        {
            sendError(
                requestID: message["requestID"] as? String, code: "runtime",
                message: "Render snapshot exceeds the 1 MB limit.")
            return
        }
    ...
    private func enforceBudgets(startedAt: Date, requestID: String?) {
        ...
        if Date().timeIntervalSince(startedAt) > Self.maxEvaluationDuration {
            sendError(
                ...
                message: "JavaScript evaluation exceeded the 2 second host budget.")
            dispose(reason: "timeout")
            return
        }
        if let residentMemory = residentMemoryBytes(), residentMemory > Self.maxResidentMemoryBytes {
            sendError(
                requestID: requestID, code: "runtime", message: "Extension host exceeded the 96 MB memory budget.")
    ...
    private func dispose(reason: String, emit: Bool = true) {
        ...
        if let jsContext = context, let disposeFunction = jsContext.objectForKeyedSubscript("__opencastDispose"),
            disposeFunction.isObject
        {
    ...
    private static let bootstrap = """
        (function () {
          ...
          globalThis.__opencast = {
            ...
            commandName: globalThis.__opencastCommandName || "",
            extensionName: globalThis.__opencastExtensionName || "",
            commandMode: globalThis.__opencastCommandMode || "view",
            launchType: globalThis.__opencastLaunchType || "userInitiated",
            supportPath: globalThis.__opencastSupportPath || "",
            assetsPath: globalThis.__opencastAssetsPath || "",
            ...
            feedback: function (kind, payload) {
              send(Object.assign({ type: "feedback", feedback: kind }, payload || {}));
            },
            navigation: function (action, component) {
              send({ type: "navigation", action: action, component: component || null });
            },
            requestCapability: function (capability, payload, onProgress) {
              ...
              keepForProgress: !!(payload && ((payload.options && payload.options.stream) || payload.stream))
                };
          ...
          globalThis.__opencastDispose = function () {
            ...
            if (typeof globalThis.__opencastUnmount === "function") globalThis.__opencastUnmount();
