...
How Bettercast is wired together. See the per-subsystem docs for internals:
...
`BettercastApp` (`@main`) declares only a `MenuBarExtra` scene; everything else visible is driven
