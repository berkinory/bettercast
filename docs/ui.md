...
- **`FeatureIcon`** is the shared identity-icon renderer: commands, Settings navigation, Settings controls, the Clipboard footer mode badge, shortcut targets, and in-window menu symbols all use its 14pt monochrome glyph, shared tile, and system glass treatment. Emoji identity icons use a real emoji glyph. Clipboard content-row thumbnails intentionally retain their neutral row tile. Size changes express hierarchy; surface treatment does not.
- **`SettingsControlRow`** uses a 28pt `FeatureIcon` tile, medium callout title, optional two-line caption, and intrinsic-width trailing control. Feature tint arguments color the single-tone icon tile and glyph; generic chrome stays neutral.
