...
- Feature controls do not have to collapse into rows: Launcher uses visual Standard/Compact tiles, Clipboard uses app chips, Emoji uses direct tone swatches, Hyper Key shows physical-key → modifier mapping, and per-item shortcuts use an aligned table with eye buttons instead of a wall of switches. Their selection surfaces stay neutral.
