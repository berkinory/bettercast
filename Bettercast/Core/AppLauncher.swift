...
enum AppLauncher {
    ...
    /// What Quit All acts on: every app with a Dock presence, minus Finder and Bettercast itself. Accessories and background agents are left alone. The caller resolves this once and terminates that same list, so the set it confirms is the set it quits.
    @MainActor
    static func quitAllTargets() -> [NSRunningApplication] {
        // Excluded by PID, not by activation policy: About/Settings temporarily flips Bettercast to `.regular`, which a policy-only filter would read as a target.
