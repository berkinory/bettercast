...
private enum CapsLockRemap {
    ...
    private static let queue = DispatchQueue(label: "com.bettercast.capslock-remap", qos: .utility)
    ...
    private static func apply(_ mapping: String) {
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                NSLog("Bettercast: hidutil remap exited %d", process.terminationStatus)
            }
        } catch {
            NSLog("Bettercast: hidutil caps lock remap failed: %@", error.localizedDescription)
        }
    }
}
